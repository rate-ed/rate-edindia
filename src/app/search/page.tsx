    ...
    async function load() {
      ...
      try {
        ...
        if (selectedSubjects.length === 0) {
          ...
          setTeachers(all.filter((t: Teacher) => 
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      <div className="text-center mb-16">
        <h1 className="text-5xl font-black text-[#13A699] uppercase tracking-tighter mb-4 leading-none">Find Your Expert</h1>
          ...
          <div className="grid grid-cols-1 gap-6">
            {teachers.map((t: Teacher) => (
              <Link href={`/book/${t.id}`} key={t.id} className="bg-white p-8 rounded-[2.5rem] shadow-xl border-2 border-[#FFD708]/10 no-underline flex items-center gap-8 hover:border-[#13A699]/40 transition-all">
                ...
                <div>
                  <h3 className="text-3xl font-black text-black uppercase mb-1 leading-none">{t.user?.name || 'Expert'}</h3>
